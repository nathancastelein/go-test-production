...
func (l *LoggerMiddlewareSuite) TestContextWithRequestId() {
	// Arrange
	middleware := Logger()(func(c echo.Context) error {
		// Act
		requestId, err := GetRequestIDFromContext(c.Request().Context())

		// Assert
		l.Require().NoError(err)
		l.Require().Equal(l.request.Header.Get("X-Request-Id"), requestId)
		return nil
	})
	err := middleware(l.ctx)
	l.Require().NoError(err)
